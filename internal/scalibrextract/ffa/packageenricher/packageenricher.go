...
func (p PackageEnricher) Enrich(ctx context.Context, input *enricher.ScanInput, inv *inventory.Inventory) error {
	...
	for _, pkg := range inv.Packages {
		...
		g.Go(func() error {
			...
			st := spanner.NewStatement("select Key from DiscriminantHashesTest where SHA256Sum=(@Hash)")
			st.Params["Hash"] = hash

			pkgMatch := PackageMatch{
				VersionIntersection: map[string]struct{}{},
			...
			err := iter.Do(func(row *spanner.Row) error {
				var val string
				err := row.ColumnByName("Key", &val)
				if err != nil {
					return err
				}

				key, err := parseSpannerKey(val)
				if err != nil {
					return err
				}

				pkgMatch.PkgName = key["apt.p"]
				pkgMatch.VersionIntersection[key["apt.v"]] = struct{}{}
	...
	for pkgName, pkgMatch := range pkgMatches {
		if _, ok := statusPkgs[pkgName]; !ok {
			if len(pkgMatch.VersionIntersection) == 1 {
				var version string
				for ver := range pkgMatch.VersionIntersection {
					version = ver
				}
				newPackages = append(newPackages, &extractor.Package{
					...
					Version:   version,
					Locations: nil,
					Extractor: hashextractor.Extractor{},
				})
			}
