...
- **New extractors and plugin support via `osv-scalibr`**:
  - Add Electron ASAR archive extractor (`javascript/electronasar`) to scan Electron apps and archives (`.asar`) ([google/osv-scalibr#2141](https://github.com/google/osv-scalibr/pull/2141)).
  - Add Jupyter Notebook package installation extractor (`python/ipynb`) to extract dependencies from `.ipynb` files installed via pip, uv add, conda, mamba, etc. ([google/osv-scalibr#2047](https://github.com/google/osv-scalibr/pull/2047)).
  - Add Bazel aspect extractor (`bazel/aspect`) to extract dependencies from Bazel workspaces, normalizing bzlmod and gazelle mangled names to native ecosystem PURLs ([google/osv-scalibr#2354](https://github.com/google/osv-scalibr/pull/2354)).
  - Add US ITIN secret detector (`secrets/itin`) ([google/osv-scalibr#2283](https://github.com/google/osv-scalibr/pull/2283)).
- **Dependency scanning & lockfile improvements via `osv-scalibr`**:
  ...
  - Extract package identity and direct dependencies from `pyproject.toml` manifests and Python wheel metadata.
  - Support PNPM lockfiles with a separate YAML document for `devEngines` metadata.
...
- [Bug #3055](https://github.com/google/osv-scanner/pull/3055) Fix `--help` flag race condition by initializing global variables in `init()`.
...
- **Fixes via `osv-scalibr`**:
  ...
  - Prevent npm lifecycle script execution (`--ignore-scripts`) in guided remediation TUI ([google/osv-scalibr#2392](https://github.com/google/osv-scalibr/pull/2392)).
  ...
  - Handle GCFS layer directories cleanly during container scanning on GKE Image Streaming clusters.
...
- Remove unused `internal/depsdev` package ([#3069](https://github.com/google/osv-scanner/pull/3069)).
